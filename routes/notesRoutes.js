...
const { getNotesByGroupe, upsertNote, getMesAffectations, getBilanEnseignant, getSessionForTeacher, soumettreNotesGroupe } = require('../controllers/notesController');
...
// POST /api/notes/soumettre
// Soumission des notes d'un groupe TD/TP au coordonnateur (verrouillage)
router.post('/soumettre', verifierToken, soumettreNotesGroupe);

// GET /api/notes/session-active?semestre=S1
// Read-only session state for teachers (NORMALE / RATTRAPAGE)
router.get('/session-active', verifierToken, getSessionForTeacher);

// GET /api/notes/bilan?semestre=S5
// Délibérations en lecture seule — scopé aux modules/groupes de l'enseignant
