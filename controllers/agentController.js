...
const affecterEnseignant = async (req, res) => {
    try {
        ...
        // ── Contrôle de grade LMD : CM réservé aux MAA et plus ──
        // Les grades inférieurs (MAB, Assistant) ne peuvent pas enseigner en CM
        // ni être désignés comme responsable de matière
        if (type_seance === 'CM') {
            const [ensRows] = await db.query(
                'SELECT grade FROM utilisateurs WHERE id_utilisateur = ?',
                [id_utilisateur]
            );
            if (ensRows.length > 0) {
                const grade = ensRows[0].grade;
                const gradesInterdits = ['MAB', 'Assistant', 'Maître Assistant B'];
                if (gradesInterdits.includes(grade)) {
                    return res.status(403).json({
                        message: `Restriction LMD : un enseignant de grade "${grade}" ne peut pas assurer un cours magistral (CM). Le grade minimum requis est MAA.`
                    });
                }
            }
        }
        ...
        // CM → automatiquement responsable matière ; TD/TP → non
        const estResponsable = type_seance === 'CM' ? 1 : 0;

        let sql = "INSERT INTO affectations (id_utilisateur, id_module, id_groupe, annee_univ, type_seance, section, niveau, est_responsable_matiere) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        await db.query(sql, [id_utilisateur, id_module, id_groupe, annee_univ, type_seance, section || null, niveau || null, estResponsable]);
...
// ============================================================
// Gestion des responsables matière (coordonnateurs)
// L'agent peut assigner/retirer le rôle de responsable
// sur les affectations CM d'un enseignant
// ============================================================
const getEnseignantModules = async (req, res) => {
    try {
        const id = req.params.id;
        ...
        console.error("Erreur getEnseignantModules:", err);
        res.status(500).json({ message: "Erreur chargement modules de l'enseignant." });
    }
};

const toggleResponsableMatiere = async (req, res) => {
    try {
        const { id_affectation, est_responsable } = req.body;

        if (!id_affectation) {
            return res.status(400).json({ message: "id_affectation est obligatoire." });
        }

        // Vérifier que l'affectation existe et récupérer ses infos
        const [aff] = await db.query(
            `SELECT a.id_affectation, a.id_module, a.id_utilisateur, a.type_seance, u.grade
             FROM affectations a
             JOIN utilisateurs u ON a.id_utilisateur = u.id_utilisateur
             WHERE a.id_affectation = ?`,
            [id_affectation]
        ...
        const affectation = aff[0];

        // Seules les affectations CM peuvent être désignées responsable
        if (affectation.type_seance !== 'CM') {
            return res.status(400).json({
                message: "Seul un enseignant affecté en CM peut être désigné responsable de matière."
            });
        }

        // Vérification du grade : MAA et supérieur uniquement
        const gradesAutorises = ['MAA', 'MCA', 'MCB', 'Prof', 'Professeur'];
        if (est_responsable && !gradesAutorises.includes(affectation.grade)) {
            return res.status(400).json({
                message: `Le grade ${affectation.grade || 'inconnu'} ne permet pas d'être coordonnateur. Requis : MAA ou supérieur.`
            });
        }

        const newVal = est_responsable ? 1 : 0;

        // Si on active le responsable, on retire d'abord les autres responsables du même module
        if (newVal === 1) {
            await db.query(
                `UPDATE affectations SET est_responsable_matiere = 0 
                 WHERE id_module = ? AND est_responsable_matiere = 1`,
                [affectation.id_module]
            );
        }

        await db.query(
            `UPDATE affectations SET est_responsable_matiere = ? WHERE id_affectation = ?`,
            [newVal, id_affectation]
        );

        res.json({
            message: est_responsable
                ? "Enseignant désigné comme coordonnateur de matière."
                : "Rôle de coordonnateur retiré.",
            est_responsable_matiere: newVal
        });
    } catch (err) {
        console.error("Erreur toggleResponsableMatiere:", err);
