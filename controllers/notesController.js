    ...
    try {
        ...
        if (hasTD) {
            ...
            const [aff] = await db.query(
                `SELECT id_affectation, periode_saisie_ouverte, statut_saisie 
                 FROM affectations 
            ...
            if (aff[0].statut_saisie === 'SOUMIS') {
                return res.status(403).json({ message: "Notes TD déjà soumises au coordonnateur. Contactez le responsable de matière pour déverrouiller." });
            }
        }
        ...
        if (hasTP) {
            ...
            if (aff[0].statut_saisie === 'SOUMIS') {
                return res.status(403).json({ message: "Notes TP déjà soumises au coordonnateur. Contactez le responsable de matière pour déverrouiller." });
            }
        }
...
// ============================================================
// soumettreNotesGroupe
// Un enseignant TD/TP soumet ses notes — les verrouille en "SOUMIS"
// Seul le responsable matière pourra les déverrouiller
// ============================================================
const soumettreNotesGroupe = async (req, res) => {
    try {
        ...
        // Vérifier que l'affectation appartient à l'enseignant connecté
        const [rows] = await db.query(
            `SELECT id_affectation, type_seance, statut_saisie
             FROM affectations
             WHERE id_affectation = ? AND id_utilisateur = ?`,
            [id_affectation, id_enseignant]
        );
        if (rows.length === 0) {
            return res.status(404).json({ message: "Affectation introuvable ou non autorisée." });
        }

        const aff = rows[0];

        // Seuls les TD/TP peuvent soumettre (le CM est responsable)
        if (aff.type_seance === 'CM') {
            return res.status(400).json({ message: "Le responsable CM n'a pas besoin de soumettre ses notes." });
        }

        if (aff.statut_saisie === 'SOUMIS') {
            return res.status(400).json({ message: "Notes déjà soumises." });
        }

        // Verrouiller
        await db.query(
            "UPDATE affectations SET statut_saisie = 'SOUMIS' WHERE id_affectation = ?",
            [id_affectation]
        );

        res.json({ message: "Notes soumises avec succès. Elles sont maintenant verrouillées en attente de validation par le coordonnateur." });
    } catch (err) {
        console.log("Erreur dans soumettreNotesGroupe:", err);
        res.status(500).json({ message: "Erreur lors de la soumission des notes." });
    }
};

module.exports = { getNotesByGroupe, upsertNote, getMesAffectations, getBilanEnseignant, getSessionForTeacher, soumettreNotesGroupe };
